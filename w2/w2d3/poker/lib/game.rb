...
class Game
  ...
  def take_bets
    ...
    until no_raises
      ...
      pivoted_players.each_with_index do |player, idx|
        next if player.folded?
        ...
        when :bet
          ...
          bet = player.get_bet
          raise "bet must be at least $#{high_bet}" if bet < high_bet
  ...
  def end_round
    show_hands
    p "#{winner.hand} wins: $#{pot}"
    p "winner hand: #{winner.hand.rank}"
  ...
  def show_hands
    puts "HANDS"
    @players.each_with_index do |player, _|
      next if player.folded?
