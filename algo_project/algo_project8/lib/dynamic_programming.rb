...
class DPProblems
  ...
  def make_change(amt, coins, coin_cache = { 0 => 0 })
    ...
    while count < coins.length && coins[count] <= amt
      num_change = 1 + make_change(amt - coins[count], coins, coin_cache)
      if num_change.is_a?(Integer)
        valid = true
        min_change = num_change if min_change > num_change
      end
      count += 1
    end
  ...
  def knapsack(weights, values, capacity)
    ...
    solution_table = knapsack_step(weights, values, capacity)
    solution_table[capacity][weights.length - 1]
  end

  def knapsack_step(weights, values, capacity)
    solution_table = []
